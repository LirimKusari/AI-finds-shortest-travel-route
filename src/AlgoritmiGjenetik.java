...
public class AlgoritmiGjenetik extends JPanel {
    ...
    // public static PoissonDistribution pusson = new PoissonDistribution(0.4);
    //public static CauchyDistribution cauchy = new CauchyDistribution();
    ...
    private int madhesiaFillestarePopullates=300;
    private int numriMaksimalIterimeve=600;
    ...
    private Image harta;
    private boolean vizatoHarten=true;


    public AlgoritmiGjenetik(int numriVeturave, Point[] pikat,Point depoja) {
        this.depoja=depoja;
        try {
        ...
        for (int i = 0; i < madhesiaFillestarePopullates; i++) {//krijon popullaten fillestare
    ...
    public void evolucioni() {
        ...
        while (a < numriMaksimalIterimeve && (popullata.get(popullata.size()-1).score-popullata.get(0).score>10)) {//300 gjenerata ose shkuje naj kusht tjeter per mbarim psh ndryshimi i poenave mes individid tpar edhe tfundit mu kon 50 a qashtu dishka
            ...
            sortPopullaten();
            while (popullata.size() > madhesiaFillestarePopullates) {
    ...
    public void paintComponent(Graphics g2) {//e vizaton gjendjen ma tmire per momentin
        ...
        if(vizatoHarten)
        {g.drawImage(harta,screenDimensions.width/2-screenDimensions.height/2,0,screenDimensions.height,screenDimensions.height,null,null);}
    ...
    public static void main(String[] args) {
        ...
        AlgoritmiGjenetik obj = new AlgoritmiGjenetik(8, Points,new Point(screenDimensions.width/2,screenDimensions.height/2));//numri i veturave pe rregullohet prej sveti se pe mujin mu kone gjenet edhe me ka 0 qytete po qikjo osht vetem limiti i siperm
        obj.vizatoHarten=false;
